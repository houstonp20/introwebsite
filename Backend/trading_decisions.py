...
import numpy as np
import logging
logging.basicConfig(filename='/Users/houstonprewett/Desktop/Python/Alpaca/script.log', level=logging.INFO, format='%(asctime)s %(levelname)s: %(message)s')
...
def check_proximity_to_fib(stock_data, stock, fib_df, trend):
    logging.info("Calculating Fibonacci Numbers")
    if trend == 'negative':
...
def check_stochastic_intersections_and_SMA(stock_data, stocks):
    logging.info("Checking Indicator Signals")
    signals = []
